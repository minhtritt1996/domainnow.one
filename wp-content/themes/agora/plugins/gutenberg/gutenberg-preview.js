...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			agora_create_observer( 'agora-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				agora_gutenberg_editor_init();
			} );
		} else {
		 	agora_gutenberg_editor_init();
		}
	...
	function agora_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length && editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'agora_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'agora_inited' ) ) {
	...
	// Init on page load
	function agora_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = agora_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + AGORA_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + AGORA_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in AGORA_STORAGE['override_classes'] ) {
				$obj.addClass( AGORA_STORAGE['override_classes'][i].replace( '%s', AGORA_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( agora_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && AGORA_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'agora_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			agora_remove_observer( 'agora-check-editor-styles-wrapper' );
			agora_create_observer( 'agora-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'agora_inited' );
	}
} );
