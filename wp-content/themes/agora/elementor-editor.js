...
jQuery( document ).ready(
	function() {
		...
		agora_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="agora_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.agora_post_editor' );
						if ( link.length > 0 ) {
							agora_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.agora_post_editor', function(e) {
						agora_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('agora_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function agora_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
