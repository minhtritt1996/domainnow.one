/* global agora_color_schemes, agora_dependencies, Color */
...
( function( api ) {
	...
	for (var i in agora_color_schemes) {
		cssTemplate['scheme_' + i] = wp.template( 'agora-color-scheme-' + i );
	}
	// Add Template with theme fonts
	cssTemplate['theme_fonts'] = wp.template( 'agora-fonts' );
	// Add Template with theme vars
	cssTemplate['theme_vars'] = wp.template( 'agora-vars' );
	...
	api.bind( 
		'ready', function() {
			...
			jQuery( '#customize-header-actions #save' )
				.before( '<input type="button" class="button customize-action-reset" value="' + agora_customizer_vars['msg_reset'] + '">' );
			jQuery( '#customize-header-actions .customize-action-reset' )
				.on( 'click', function(e) {
					if (confirm( agora_customizer_vars['msg_reset_confirm'] )) {
			...
			jQuery( '#customize-header-actions .spinner' )
				.after( '<button class="button customize-action-refresh icon-spin3" title="' + agora_customizer_vars['msg_reload_title'] + '">' + agora_customizer_vars['msg_reload'] + '</button>' );
			...
			if (agora_customizer_vars && agora_customizer_vars['theme_name_suffix']) {
				jQuery( '#customize-info .site-title' ).append( agora_customizer_vars['theme_name_suffix'] );
			}
			...
			jQuery( '#customize-theme-controls [id^="customize-control-load_fonts"]' )
				.on( 'change', agora_customizer_update_load_fonts );
			...
			jQuery( '#customize-theme-controls .control-section > .accordion-section-title' )
				.on( 'click', function() {
					...
					if (id !== '') {
						...
						if (section.length > 0) {
							agora_customizer_check_dependencies( section );
						}
					}
				} );

			// Decorate subsections in the panel 'Typography'
			if ( agora_customizer_vars['decorate_fonts'] ) {

				jQuery('body').addClass( 'agora_decorate_fonts_section' );

				// Prepare section for open on title is clicked
				var agora_customizer_title_clicked = function( title ) {
					var id = title.parent().attr( 'aria-owns' );
					agora_customizer_close_opened_sections( title.parents( '#sub-accordion-panel-fonts' ), id );
					var section = jQuery( '#' + id ),
					...
					.each ( function() {
						...
						$self
							.on( 'keydown', function(e) {
								if ( e.which == 13 ) {
									agora_customizer_title_clicked( $self );
								}
							} )
							.on( 'click', function() {
								agora_customizer_title_clicked( $self );
				...
				var agora_customizer_close_opened_sections = function( panel, current_id ) {
					if ( panel.length ) {
				...
				jQuery( '#customize-theme-controls #sub-accordion-panel-fonts .customize-panel-back' )
					.on( 'keydown', function(e) {
						if ( e.which == 13 ) {
							agora_customizer_close_opened_sections( jQuery(this).parents('#sub-accordion-panel-fonts') );
						}
					} )
					.on( 'click', function() {
						agora_customizer_close_opened_sections( jQuery(this).parents('#sub-accordion-panel-fonts') );
	...
	api.bind( 'change', function(obj) {
		...
		agora_customizer_check_dependencies( jQuery( '#customize-theme-controls #customize-control-' + obj.id ).parents( '.control-section' ) );
		agora_customizer_refresh_preview( obj );
	} );
	...
	api.bind( 'refresh_schemes', function() {
		...
		for (i in cssTemplate) {
			...
			i = i.replace( 'scheme_', '' );
			if (typeof agora_color_schemes[i] === 'undefined') {
				delete cssTemplate['scheme_' + i];
				jQuery( '#tmpl-agora-color-scheme-' + i ).remove();
			} else if (tpl_idx == '') {
				tpl_idx     = i;
				tpl_content = jQuery( '#tmpl-agora-color-scheme-' + i ).html().trim();
			}
		}
		// Add new templates
		var regex = new RegExp( "\.scheme_" + tpl_idx, "g" );
		for (i in agora_color_schemes) {
			if (typeof cssTemplate['scheme_' + i] === 'undefined') {
				jQuery( '#tmpl-agora-color-scheme-' + tpl_idx )
				.clone()
				.attr( 'id', 'tmpl-agora-color-scheme-' + i )
				.html( tpl_content.replace( regex, '.scheme_' + i ) )
				.insertAfter( jQuery( '#tmpl-agora-color-scheme-' + tpl_idx ) );
				cssTemplate['scheme_' + i] = wp.template( 'agora-color-scheme-' + i );
			}
		}
	} );
	...
	for (var action in agora_customizer_vars['actions']['expand']) {
		if (action == 'length') {
			continue;
		}
		if (agora_customizer_vars['actions']['expand'][action]['type'] == 'panel') {
			api.panel(
				action, function( panel ) {
					panel.expanded.bind(
						function( isExpanded ) {
							if ( isExpanded ) {
								var data = agora_customizer_vars['actions']['expand'][panel.id];
			...
			api.section(
				action, function( section ) {
					section.expanded.bind(
						function( isExpanded ) {
							if ( isExpanded ) {
								var data = agora_customizer_vars['actions']['expand'][section.id];
								if (typeof data['url'] !== 'undefined' && data['url'] !== '') {
									api.previewer.previewUrl.set( data['url'] );
								}
								if (typeof data['callback'] !== 'undefined' && data['callback'] !== '' && typeof window[data['callback']] === 'function') {
	...
	function agora_customizer_get_field_value(fld) {
	...
	function agora_customizer_check_dependencies(cont) {
		if ( typeof agora_dependencies == 'undefined' || AGORA_STORAGE['check_dependencies_now'] ) return;
			...
			function() {
				...
				if (depend) {
					...
					for (i in depend) {
						...
						if (fld.length > 0) {
							val = agora_customizer_get_field_value( fld );
							if ( val == 'inherit' ) {
								dep_cnt = 0;
								dep_all = 1;
								var tag;
	...
	function agora_customizer_refresh_preview(obj) {
		...
		if (updateCSS) {
			...
			if (id == 'scheme_storage') {
				...
				// Any color in the scheme_storage is changed
			} else if (Object.values( agora_sorted_schemes ).indexOf( id ) >= 0) {
				forceUpdateCSS = true;
				...
				if (var_name !== undefined) {
					...
					agora_customizer_update_theme_vars( jQuery( '[data-customize-setting-link="' + id + '"]' ).data( 'var_name' ), val );
					processed = true;
				}

				// Any theme fonts parameter is changed
			} else {
				for (opt in agora_theme_fonts) {
					for (rule in agora_theme_fonts[opt]) {
						if (opt + '_' + rule == id) {
							// Store new value to the fonts table
							agora_customizer_update_theme_fonts( opt, rule, val );
							processed = true;
							break;
						}
					}
	...
	// Actions buttons
	function agora_customizer_actions(e) {
	...
	function agora_customizer_update_theme_vars(opt, value) {
		agora_theme_vars[opt] = parseFloat( value );
	}

	// Store new value in the theme fonts
	function agora_customizer_update_theme_fonts(opt, rule, value) {
		agora_theme_fonts[opt][rule] = value;
	}

	// Change theme fonts options if load fonts is changed
	function agora_customizer_update_load_fonts() {
		var opt_list = [], i, tag, sel, opt, name = '', family = '', val = '', new_val = '', sel_idx = 0;
		updateCSS    = false;
		for (i = 1; i <= agora_customizer_vars['max_load_fonts']; i++) {
	...
	function agora_customizer_update_css() {
		...
		var vars = agora_customizer_add_theme_vars( agora_theme_vars );

		// Make styles and add its to the css
		css += agora_customizer_prepare_html_value( cssTemplate['theme_vars']( vars ) );

		// Add theme specific fonts rules
		var fonts = agora_customizer_add_theme_fonts( agora_theme_fonts );

		// Make styles and add its to the css
		css += agora_customizer_prepare_html_value( cssTemplate['theme_fonts']( fonts ) );

		// Add colors
		var sorted = {}, scheme = '';
		for (var param in agora_sorted_schemes) {
			if (param == 'length') {
				continue;
			}
			scheme = api( agora_sorted_schemes[param] )();
			if (scheme && scheme !== 'inherit') {
				sorted[scheme] = 1;
			}
		}
		for (scheme in agora_color_schemes) {
			sorted[scheme] = 1;
		}
		for (scheme in sorted) {
			var i, colors = [];
			// Copy all colors to the temp var
			for (i in agora_color_schemes[scheme].colors) {
				colors[i] = agora_color_schemes[scheme].colors[i];
			}
			// Add theme specific colors and tints
			colors = agora_customizer_add_theme_colors( colors );
			...
			var tmpl = jQuery( '#tmpl-agora-color-scheme-' + scheme ).html().trim();
			for (i in colors) {
	...
	function agora_customizer_add_theme_colors(colors) {
		if (agora_additional_colors) {
			var clr = '', v = '';
			for (var k in agora_additional_colors) {
				v   = agora_additional_colors[k];
				clr = colors[v['color']];
	...
	function agora_customizer_add_theme_fonts(fonts) {
		var rez = [];
		for (var tag in fonts) {
			...
			rez[tag + '_font-size']       = typeof fonts[tag]['font-size'] != 'undefined'
									&& fonts[tag]['font-size'] != 'inherit'
												? 'font-size:' + agora_customizer_prepare_css_value( fonts[tag]['font-size'] ) + ";"
												: '';
			...
			rez[tag + '_margin-top']      = typeof fonts[tag]['margin-top'] != 'undefined'
									...
									&& fonts[tag]['margin-top'] != 'inherit'
												? 'margin-top:' + agora_customizer_prepare_css_value( fonts[tag]['margin-top'] ) + ";"
												: '';
			rez[tag + '_margin-bottom']   = typeof fonts[tag]['margin-bottom'] != 'undefined'
									&& fonts[tag]['margin-bottom'] !== ''
									&& fonts[tag]['margin-bottom'] != 'inherit'
												? 'margin-bottom:' + agora_customizer_prepare_css_value( fonts[tag]['margin-bottom'] ) + ";"
												: '';
		}
		return rez;
	}
	...
	function agora_customizer_add_theme_vars(vars) {
		var rez = [];
		if ( typeof vars['rad'] != 'undefined' ) {
			if (vars['rad'] == '') {
				vars['rad'] = 0;
			}
			rez['rad']      = agora_customizer_prepare_css_value( vars['rad'] );
		...
		if ( typeof vars['page_width'] != 'undefined' ) {
			...
			if ( isNaN( vars['page_width'] ) || vars['page_width'] === 0) {
				vars['page_width'] = agora_customizer_vars['page_width_default'];
			}
			vars['sidebar_width']      = parseInt( vars['sidebar_width'], 10 );
			vars['sidebar_gap']        = parseInt( vars['sidebar_gap'], 10 );

			rez['page_width']          = agora_customizer_prepare_css_value( vars['page_width'] );
			rez['page_boxed_extra']    = agora_customizer_prepare_css_value( vars['page_boxed_extra'] );
			rez['page_fullwide_extra'] = agora_customizer_prepare_css_value( vars['page_fullwide_extra'] );
			rez['page_fullwide_max']   = agora_customizer_prepare_css_value( vars['page_fullwide_max'] );
			rez['grid_gap']            = agora_customizer_prepare_css_value( vars['grid_gap'] );
			rez['sidebar_prc']         = vars['sidebar_width'] / vars['page_width'];
			rez['sidebar_gap_prc']     = vars['sidebar_gap'] / vars['page_width'];
			rez['sidebar_width']       = agora_customizer_prepare_css_value( vars['sidebar_width'] );
			rez['sidebar_gap']         = agora_customizer_prepare_css_value( vars['sidebar_gap'] );
			rez['sidebar_gap_width']   = agora_customizer_prepare_css_value( vars['sidebar_gap'] );
		}
		return rez;
	}

	// Add ed to css value
	function agora_customizer_prepare_css_value(val) {
		if (val !== '' && val != 'inherit') {
			var ed = ('' + val).substr( -1 );
			if ('0' <= ed && ed <= '9') {
				val += 'px';
	...
	function agora_customizer_prepare_html_value(val) {
