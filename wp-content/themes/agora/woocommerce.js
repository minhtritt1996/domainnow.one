...
jQuery( document ).on( 'action.ready_agora', function() {
	...
	$document.on( 'action.resize_agora', function() {
		_list_products_height = $list_products.length > 0 ? $list_products.height() : 0;
	} );
	...
	// Change display mode
	//--------------------------------
	jQuery( '.woocommerce,.woocommerce-page' ).on( 'click', '.agora_shop_mode_buttons a', function(e) {
		var $self = jQuery(this),
			mode  = $self.hasClass( 'woocommerce_thumbs' ) ? 'thumbs' : 'list';
		agora_set_cookie( 'agora_shop_mode', mode, 365 * 24 * 60 * 60 * 1000 );   // 1 year
	...
	// Add buttons inc/dec
	function agora_woocommerce_add_inc_dec() {
			...
			if ( $self.find('>input[name="quantity"][type="hidden"]').length === 0 ) {
				...
				$self
					.on( 'click', '>span', function(e) {
						agora_woocomerce_inc_dec_click( jQuery( this ) );
						e.preventDefault();
						return false;
					} );
			} else {
	...
	function agora_woocomerce_inc_dec_click(button) {
	...
	$document.on( 'action.scroll_agora', function(e) {
		...
		if ( $list_products.length > 0 && $list_products.offset().top + _list_products_height < agora_window_scroll_top() + agora_window_height() * 1.5 ) {
	...
	jQuery( document.body ).on( 'updated_wc_div', function() {
		...
		agora_create_observer( 'update_woocommerce_cart', table_cart.parents('form').eq(0), function( mutationsList ) {
			for ( var mutation of mutationsList ) {
				if ( mutation.type == 'childList' || mutation.type == 'subtree' ) {
					agora_remove_observer( 'update_woocommerce_cart' );
					table_cart = jQuery( 'table.cart' );
					agora_woocommerce_update_cart( 'update' );
					break;
				}
			}
		} );
	...
	if ( table_cart.length > 0 ) {
		agora_woocommerce_update_cart( 'init' );
	}

	// Resize action
	$window.on( 'resize', function() {
		if ( table_cart.length > 0 ) {
			agora_woocommerce_update_cart( 'resize' );
		}
	} );

	// Update cart display
	function agora_woocommerce_update_cart(status){
		setTimeout(
			function() {
				var w = window.innerWidth;
				if (w == undefined) {
					w = agora_window_width() + ( agora_window_height() < $document.height() || agora_window_scroll_top() > 0 ? 16 : 0 );
				}
				...
				if ( w >= AGORA_STORAGE['mobile_layout_width'] && status == 'resize' && jQuery( 'table.cart .mobile_cell' ).length > 0) {
					if ( tbl.length > 0 ) {
